...
export default function PlayPage() {
  ...
  useEffect(() => {
    ...
    const savedPlayerId = sessionStorage.getItem('arena_player_id');
    const savedPin = sessionStorage.getItem('arena_player_pin');
    const savedName = sessionStorage.getItem('arena_player_name');
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    if (!pin || pin.length < 4) {
      setErrorMessage('Vui lòng nhập mã PIN 4 chữ số hợp lệ!');
      return;
    }
    if (!playerName.trim()) {
    ...
    <div className="min-h-screen flex flex-col justify-between">
      ...
      <main className="flex-1 max-w-md w-full mx-auto px-4 py-6 flex flex-col justify-center">
        {/* =========================================================================
           ...
           ========================================================================= */}
        {!currentPlayer && (
          <div className="p-6 rounded-2xl academic-card border-slate-700/80 bg-slate-900/90 shadow-xl">
            ...
            <form onSubmit={handleJoin} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="1234"
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-700 text-amber-400 font-mono font-bold text-xl tracking-widest text-center focus:outline-none focus:border-amber-500"
                  required
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-amber-600 hover:bg-amber-500 disabled:opacity-40 text-slate-950 font-bold text-sm uppercase tracking-wider transition-colors shadow-sm"
              >
        ...
        {currentPlayer && room && room.status === 'LOBBY' && (
          <div className="p-6 rounded-2xl academic-card border-slate-700/80 bg-slate-900/90 text-center">
            <div className="text-3xl mb-2">{TEAMS[currentPlayer.teamId]?.icon}</div>
            ...
            <h2 className="text-xl font-serif font-bold text-white mt-2.5">{currentPlayer.name}</h2>
            <p className="text-xs text-slate-400 italic mt-0.5">
              "{TEAMS[currentPlayer.teamId]?.slogan}"
            </p>

            <div className="my-6 p-3 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-center space-x-2.5">
        ...
        {currentPlayer && room && room.status === 'QUESTION' && currentQ && (
          <div className="flex-1 flex flex-col justify-between py-1">
            <div className="flex items-center justify-between p-2.5 rounded-xl academic-card border-slate-800 mb-3">
            ...
            <div className="p-4 rounded-xl academic-card border-slate-700/80 bg-slate-900/80 mb-3">
              <span className="text-[10px] font-semibold uppercase text-slate-400">
                {currentQ.round === 1
                  ? `Vòng 1 · Câu ${room.currentQuestionIndex + 1}/5`
                  : `Vòng 2 · Tình huống ${room.currentQuestionIndex - 4}/5`}
              </span>
              <h3 className="text-sm sm:text-base font-serif font-bold text-slate-100 mt-1 leading-snug">
                {currentQ.text}
              </h3>
            </div>
              ...
              {currentQ.options.map((opt, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`p-3.5 rounded-xl border text-left transition-colors flex items-center space-x-2.5 ${
                      isSelected
            ...
            {(selectedAnswer !== null || currentPlayer.lastAnswer !== undefined) && (
              <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-700 text-center text-slate-300 text-xs font-medium">
        ...
        {currentPlayer && room && room.status === 'REVEAL' && currentQ && (
          <div className="p-6 rounded-2xl academic-card border-slate-700/80 bg-slate-900/90 text-center">
            {currentPlayer.isCorrect ? (
              <>
                <div className="w-12 h-12 rounded-full bg-emerald-950/70 border border-emerald-500 flex items-center justify-center mx-auto mb-2 text-2xl">
                  ✓
                </div>
                ...
                <div className="my-5 p-3.5 rounded-xl bg-slate-950 border border-slate-800">
              ...
              <>
                <div className="w-12 h-12 rounded-full bg-red-950/70 border border-red-500 flex items-center justify-center mx-auto mb-2 text-2xl">
                  ✕
                </div>
                ...
                <div className="my-5 p-3.5 rounded-xl bg-slate-950 border border-slate-800">
                  <div className="text-[10px] text-slate-400 uppercase font-semibold">ĐIỂM NHẬN ĐƯỢC</div>
                  <div className="text-2xl font-bold text-slate-500 font-mono mt-0.5">
                    +0đ
                  </div>
                </div>
              </>
        ...
        {currentPlayer && room && room.status === 'LEADERBOARD' && (
          <div className="p-6 rounded-2xl academic-card border-slate-700/80 bg-slate-900/90 text-center">
            ...
            <h3 className="text-lg font-serif font-bold text-slate-100">BẢNG TỔNG SẮP</h3>
            <p className="text-xs text-slate-400 mb-3">Điểm số hiện tại của bạn và các khối</p>
        ...
        {currentPlayer && room && room.status === 'FINISHED' && (
          <div className="p-6 rounded-2xl academic-card-gold border-amber-500/50 bg-slate-900/90 text-center">
            ...
            <div className="my-5 p-4 rounded-xl bg-slate-950 border border-amber-800/40">
